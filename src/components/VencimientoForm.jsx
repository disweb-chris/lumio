...
export default function VencimientoForm({
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      setCategoriasDisponibles(list.map((c) => c.nombre));
      if (!categoria && list.length) setCategoria(list[0].nombre);
    });
    return () => unsub();
  }, [uid]);
  ...
  useEffect(() => {
    if (editando) {
      ...
      setCategoria(editando.categoria || categoriasDisponibles[0] || "");
      if (editando.montoARS != null) {
        setMontoARS(editando.montoARS.toString());
        setMontoUSD(
          (editando.montoARS / cotizacionUSD).toFixed(2)
        );
      } else {
        setMontoUSD(editando.montoUSD?.toString() || "");
        setMontoARS(
          (editando.montoUSD * cotizacionUSD).toFixed(2)
        );
      }
    } else {
      // reset
  ...
  const handleSubmit = (e) => {
    ...
    if (tieneARS) nuevo.montoARS = ars;
    if (tieneUSD) nuevo.montoUSD = usd;

    try {
      if (editando?.id) {
        onActualizar({ id: editando.id, ...nuevo });
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md mb-6">
      ...
      {/* Descripción */}
      <div className="mb-2">
        <label className="block text-sm dark:text-gray-300 mb-1">Descripción</label>
        <input
          type="text"
          value={descripcion}
      ...
      {/* Monto ARS */}
      <div className="mb-2">
        <label className="block text-sm dark:text-gray-300 mb-1">Monto en ARS</label>
        <input
          type="number"
          value={montoARS}
      ...
      {/* Monto USD */}
      <div className="mb-2">
        <label className="block text-sm dark:text-gray-300 mb-1">Monto en USD</label>
        <input
          type="number"
          value={montoUSD}
      ...
      {/* Fecha */}
      <div className="mb-2">
        <label className="block text-sm dark:text-gray-300 mb-1">Fecha</label>
        <input
          type="date"
          value={fecha}
      ...
      {/* Método de pago */}
      <div className="mb-2">
        <label className="block text-sm dark:text-gray-300 mb-1">Método de pago</label>
        <select
          value={metodoPago}
      ...
      {metodoPago === "Tarjeta de crédito" && (
        <div className="mb-2">
          <label className="block text-sm dark:text-gray-300 mb-1">Tarjeta</label>
          <select
            value={subMetodo}
      ...
      <div className="mb-2 flex items-center gap-2">
        ...
        />
        <label className="text-sm dark:text-gray-300">Recurrente mensual</label>
      </div>
      {/* Categoría */}
      <div className="mb-4">
        <label className="block text-sm dark:text-gray-300 mb-1">Categoría</label>
        <select
          value={categoria}
